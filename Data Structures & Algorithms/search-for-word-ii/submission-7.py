...
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def searc(node,string,i,j):
            ...
            string = string + a
            board[i][j]='#'
            ischarinchildren=a in node.children
            if(ischarinchildren and node.children[a].end):
                ans.add(string)
            if(ischarinchildren):
                searc(node.children[a],string,i+1,j)
                searc(node.children[a],string,i,j+1)
                searc(node.children[a],string,i-1,j)
                searc(node.children[a],string,i,j-1)
            board[i][j]=a
            return
